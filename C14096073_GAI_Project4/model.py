...
class MNISTDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def sampling(self,j,n_samples,clipped_reverse_diffusion=True,device="cuda"):
        x_t=torch.randn((n_samples,self.in_channels,self.image_size,self.image_size)).to(device)
         # y=(x_t+1.)/2.
        #  save_image(y,"results/steps_{}.png".format(j*100+10),nrow=int(math.sqrt(n_samples))) 
        ...
        for i in tqdm(range(self.timesteps-1,-1,-1),desc="Sampling"):
            ...
            if clipped_reverse_diffusion:
             ...
             # if i%100==0:
                 # y=(x_t+1.)/2.
                 # k=i/100
                 # save_image(y,"results/steps_{}.png".format(j*100+k+1),nrow=int(math.sqrt(n_samples))) 
    ...
    @torch.no_grad()
    def sampling2(self,j,out_np,n_samples,clipped_reverse_diffusion=True,device="cuda"):
        out_np = np.tile(out_np, (n_samples, 1, 1, 1))
        x_t = torch.from_numpy(out_np).to(device)

        for i in tqdm(range(20-1,-1,-1),desc="Sampling"):
            noise=torch.randn_like(x_t).to(device)
            t=torch.tensor([i for _ in range(n_samples)]).to(device)
