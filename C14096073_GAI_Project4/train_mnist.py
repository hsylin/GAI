...
def create_mnist_dataloaders(batch_size,image_size=64,num_workers=4):
    
    preprocess=transforms.Compose([transforms.Resize(image_size),\
                                    transforms.ToTensor(),\
                                    transforms.Normalize([0.5],[0.5])]) #[0,1] to [-1,1]

    train_dataset = ImageFolder(root="./mnist_png/training", transform=preprocess)
    test_dataset = ImageFolder(root="./mnist_png/testing", transform=preprocess)
...
def main(args):
    device="cuda"
    train_dataloader,test_dataloader=create_mnist_dataloaders(batch_size=args.batch_size,image_size=64)
